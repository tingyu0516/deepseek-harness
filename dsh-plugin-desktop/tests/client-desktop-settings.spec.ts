...
describe('Desktop settings Slot registration', () => {
  ...
  it('offers Hu Tao and Furina in the Desktop appearance copy', () => {
    ...
    expect(zh.characterThemeBrightness).toBe('壁纸亮度')
    expect(zh.characterThemeOpacity).toBe('界面不透明度')
    expect(en.characterThemeBrightness).toBe('Wallpaper brightness')
    expect(en.characterThemeOpacity).toBe('Interface opacity')
    expect(source).toContain("set('characterTheme', next)")
    expect(source).toContain("set('characterThemeBrightness', next)")
    expect(source).toContain("set('characterThemeOpacity', next)")
