...
export class LAppModel extends CubismUserModel {
    ...
    const loadCubismMotion = (): void => {
      ...
      if (motionGroupCount == 0) {
        ...
        this.createRenderer(
          ...
          this.computeMaskBufferCount()
        );
        // The default single mask buffer caps the low-precision path at 36
        // clip contexts and its fallback merges every mask into one channel,
        // shearing masked pieces (skin protruding past the face, ornaments
        // vanishing). The buffer count scales with the model's own context
        // count, so rigs within the official limit keep the exact official
        // single-buffer path and only oversized rigs get extra buffers.
        this.setupTextures();
   ...
   */
  private computeMaskBufferCount(): number {
    ...
    for (let i = 0; i < counts.length; i++) {
      ...
      const list = Array.from(maskLists[i].slice(0, counts[i])).sort(
        (a, b) => a - b
      );
      seen.add(list.join(','));
    }
  ...
  public update(): void {
    ...
    // Host-provided hook: lets the embedder apply its own parameter/parts
    // overrides between the effect updaters and the core update, so the core
    // update only has to run once per frame with the final values.
    if (this.preCoreUpdateHook !== null) {
      this.preCoreUpdateHook();
    }
   ...
   */
  public preCoreUpdateHook: (() => void) | null = null;
      ...
      fetch(`${this._modelHomeDir}${motionFileName}`)
        ...
        .then(arrayBuffer => {
          ...
          if (this._motionCount >= this._allMotionCount) {
            ...
            // The buffer count scales with this model's own context count
            // (official capacity is 36 per buffer); rigs within the official
            // limit keep the exact official single-buffer path (see the
            // zero-motion branch above for the full rationale).
            this.setupTextures();
